...
//Session 04 - Homework 5
//Notes or Remarks:

// Sorting Artists using Radix Sort and Counting Sort
...
void analyzePerformance(const std::vector<Artist>& artists, const std::string& criteria) {
    ...
    bool ans = true;
    if (radixDuration.count()!=countingDuration.count()) ans = false;
    std::cout << "Radix Sort and Counting Sort produce the same result: " << (ans ? "Yes" : "No") << "\n";
}
